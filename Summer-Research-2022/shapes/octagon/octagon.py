...
class Octagon(): 
    ...
    def coordinatize(self):
        ...
        scenarios = [[pt1, pt2, pt3, pt4, pt5, pt6, pt7, pt8],
        ...
        # then continue to the next
        if (sorted_points[7] == None): # seven known points
            # each fifth point has one possible sixth point
            for i in range(0, len(scenarios)):
                scenarios[i][7] = Geometry.calculate_point_from_angle(ANGLE, scenarios[i][6], scenarios[i][5], side_length)
                
        # all eight points known handled above
